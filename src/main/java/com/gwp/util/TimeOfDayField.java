...
extends JPanel {
  ...
  private static final String       LEGAL_MILITARY2_CHARACTERS        = "01234";
   ...
   */
  public TimeOfDayField( boolean mil ) {
    this( new Date(), mil );
  }
   ...
   */
  public TimeOfDayField( long time ) {
   ...
   * @param currentTime The start time to use
   */
  public TimeOfDayField( Date time ) {
    this( time, false );
  }
   ...
   * @param mil Whether military time should be used
   */
  public TimeOfDayField( Date time, boolean mil ) {
    ...
    if ( military ) {
      ...
      MAX_HOUR            = 12;
      MIN_HOUR            = 1;
      TIME_FORMATTER      = new SimpleDateFormat("hh:mm:ss a");
      nodeSeparatorFields = new JTextField[3];
    }
    ...
    // seconds
    secondsField = createSecondsField();
    add( secondsField );
    ...
    if ( military ) {
      setToolTipText( "Must be of the format: HH:MM:SS" );
    } else {
      setToolTipText( "Must be of the format: HH:MM:SS AM" );
    }
  }
  ...
  public void addDocumentListener( DocumentListener l ) {
    ...
    secondsField.getDocument().addDocumentListener( l );
    if ( !isMilitary() ) {
  ...
  /**
   * Configures and returns a text field to serve as an editable area in the seconds portion of the time field.
   *
   * @return JTextField
   */
  private JTextField createSecondsField() {
    JTextField tf = new JTextField( NODE_LENGTH );
    tf.setDocument( new MinuteNodeDocument() );
    tf.setHorizontalAlignment( SwingConstants.CENTER );
    tf.setBorder( null );
    ...
    tf.addKeyListener( new KeyAdapter() {
      public void keyTyped( KeyEvent evt ) {
        ...
        // Build the new minute value
        int position = textSelected ? selectionStart : field.getCaretPosition();
        StringBuffer newSeconds = new StringBuffer();

        if ( position == 0 ) {
          newSeconds.append( c );
          if ( textSelected ) {
            if ( ( selectionLength < 2 ) && ( text.length() > 1 ) ) {
              newSeconds.append( text.charAt( 1 ) );
            }
          } else {
            newSeconds.append( text );
          }
        } else { // position should be 1
          if ( textSelected ) {
            if ( selectionLength < 2 ) {
              newSeconds.append( text.charAt( 0 ) );
            }
            newSeconds.append( c );
          } else {
            newSeconds.append( text );
        ...
        if ( c == ' ' ) {
          ...
          return;
        } else if ( !isLegalMinuteValue( newSeconds.toString() ) ) {
          Toolkit.getDefaultToolkit().beep();
          evt.consume();
        }
      }
   ...
   * @return
   */
  public int getSeconds() throws NumberFormatException {
  ...
  public String getText() {
    ...
    buffer.append( TIME_NODE_SEPARATOR );
    buffer.append( secondsField.getText() );
    if (!military) {
      buffer.append(" ");
  ...
  /**
   * Tests whether a value is allowed at the given position of the hour node
   * @param position
   * @param input
   * @return
   */
  ...
  /**
   * Tests whether a value is allowed at the given position of the minute node
   *
   * @param position
   * @param input
   * @return
  ...
  public void setDisabledTextColor( Color color ) {
    ...
    secondsField.setDisabledTextColor( color );
  }


  /**
   * Turns on or off editablility
   */
  ...
  /**
   * Sets the value of the seconds node
   */
  public void setSeconds( int seconds ) {
    String text = String.valueOf( seconds );
    secondsField.setText( seconds < 10 ? "0" + text : text );
    secondsField.repaint();
  }
